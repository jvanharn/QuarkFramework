 ...
 * reference between multiple aliasses of the same positions. This class aims to
 * maximize Layout class position interoperatability while providing a
 * consistent api for Application and Extensions devellopers alike.
 */
class Positions implements \IteratorAggregate {
	...
	/**
	 * Position references hashmap with title and description
	 * @var array
	 */
	protected $positions = array();
	
	/**
	 * Position aliasses hashmap
	 * @var array
	 */
	 ...
	 * 
	 * The initial position array and aliasses should at least contain the MAIN_CONTENT reference.
	 * @param array $positions Array with initial positions in the format 'REFERENCE_NAME' => ['title', 'some description']
	 * @param array $aliasses Array with initial aliasses in the format 'ALIAS_NAME' => 'REFERENCE_NAME'
	 */
	public function __construct($positions, $aliasses){
		$this->positions = $positions;
		$this->aliasses = $aliasses;
		
		// Check if the minimal default references are available
		if(!$this->exists('MAIN_CONTENT'))
			throw new \DomainException('The given position and aliasses did not contain the minimally required position references! These are: "MAIN_CONTENT".');
	}
	
	/**
	 * Add a available layout position to the list
	 * @param string $reference Internal position reference name.
	 * @param string $name Position title in human readible format.
	 * @param string $description Short discription about the position.
	 */
	...
	public function remove($reference){
		if(isset($this->positions[$reference])){
			...
			return true;
		}else if(isset($this->aliasses[$reference])){
			unset($this->aliasses[$reference]);
		}else return false;
	}
	...
	public function exists($reference, $alias=true){
		return (isset($this->positions[$reference]) || ($alias && isset($this->aliasses[$reference])));
	}
	
	/**
	 * Resolve position or alias to the proper position reference name
	 * @param string $reference
	 * @return false|string Returns false on unresovable alias or position or the resolved position on success.
	 */
	public function resolve($reference){
		if(isset($this->positions[$reference]))
			return $reference;
		else if(isset($this->aliasses[$reference]))
			return $this->aliasses[$reference];
		else return false;
	}
	...
	/**
	 * Get an array of all aliasses and their referenced keys
	 * @return array Alliass keys as name
	 */
	...
	public function getReferences(){
		return array_merge(array_keys($this->positions), array_keys($this->aliasses));
	}
	...
	public function __get($name){
		...
		else throw new \RuntimeException('Unaccessible or non-existend property accessed.');
	}
